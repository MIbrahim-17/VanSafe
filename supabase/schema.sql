...
create table if not exists public.drivers (
  ...
  vehicle_type text default 'Van' check (vehicle_type in ('Van', 'Wagon', 'Hi-Roof')),
  plate text default '',
...
-- One active van per parent.
create unique index if not exists links_one_per_parent on public.links (parent_id);
...
-- links: parent manages their own link; drivers can read links pointing at them.
drop policy if exists links_read on public.links;
create policy links_read on public.links for select
  using (auth.uid() = parent_id or auth.uid() = driver_id);
drop policy if exists links_insert on public.links;
create policy links_insert on public.links for insert with check (auth.uid() = parent_id);
drop policy if exists links_delete on public.links;
create policy links_delete on public.links for delete using (auth.uid() = parent_id);

-- reviews: readable by all; only a linked parent may write.
...
create policy reviews_insert on public.reviews for insert
  with check (
    ...
    and exists (
      ...
      where l.parent_id = auth.uid() and l.driver_id = reviews.driver_id
    )
  );
...
create policy locations_read on public.locations for select
  using (
    ...
    or exists (
      select 1 from public.links l
      where l.parent_id = auth.uid() and l.driver_id = locations.driver_id
    )
  );
...
create policy ts_read on public.tracking_sessions for select
  using (
    ...
    or exists (
      select 1 from public.links l
      where l.parent_id = auth.uid() and l.driver_id = tracking_sessions.driver_id
    )
  );
